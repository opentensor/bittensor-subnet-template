...
class BitcoinCoreCliTests(unittest.TestCase):
    ...
    def test_sending_transaction(self):
        ...
        result = execute_bitcoin_cli_command(
            self.container_name, f"createwallet {wallet_name}"
        )
        print(result.output.decode())

        # 2. Get new address
        address = (
            execute_bitcoin_cli_command(self.container_name, "getnewaddress")
            .output.decode()
            .strip()
        )
        ...
        result = execute_bitcoin_cli_command(
            self.container_name, f"generatetoaddress 101 {address}"
        )
        print(result.output.decode())
        ...
        # 4. Check balance
        balance = (
            execute_bitcoin_cli_command(self.container_name, "getbalance")
            .output.decode()
            .strip()
        )
        ...
        transaction_id = (
            execute_bitcoin_cli_command(
                self.container_name, f"sendtoaddress {address} {send_amount}"
            )
            .output.decode()
            .strip()
        )
