...
def index_block(_bitcoin_node, _graph_indexer, _graph_search, block_height):
    ...
    if time_taken > 0:
        indexlogger.info(
            "Processing transactions", extra = logger_extra_data(block_height = f"{block_height:>6}",num_transactions = formatted_num_transactions,time_taken = formatted_time_taken,tps = formatted_tps))
    else:
        indexlogger.info("Processed transactions in 0.00 seconds (  Inf TPS).", extra = logger_extra_data(block_height = f"{block_height:>6}",num_transactions = formatted_num_transactions))
...
def iterate_range(_bitcoin_node, _graph_indexer, _graph_search, start_height: int, end_height: int, in_reverse_order: bool = False):
    ...
    while (block_height - end_height) * step <= 0 and not shutdown_flag:
        if _graph_indexer.check_if_block_is_indexed(block_height):
            indexlogger.info(f"Skipping block. Already indexed.", extra = logger_extra_data(block_height = block_height))
            block_height += step
            continue
...
if __name__ == "__main__":
    ...
    indexlogger.info("BITCOIN_INDEXER_IN_REVERSE_ORDER", extra = logger_extra_data(in_reverse_order = in_reverse_order_str))
    ...
    if start_height_str is None:
        ...
        indexlogger.info(f"Indexed block height range", extra = logger_extra_data(indexed_min_block_height = indexed_min_block_height, indexed_max_block_height = indexed_max_block_height))
