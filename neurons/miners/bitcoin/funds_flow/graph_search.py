...
class GraphSearch:
    ...
    def solve_challenge(self, in_total_amount: int, out_total_amount: int, tx_id_last_8_chars: str) -> str:
        with self.driver.session() as session:
            result = session.run(
                """
                MATCH (t:Transaction {out_total_amount: $out_total_amount})
                WHERE t.in_total_amount = $in_total_amount AND t.tx_id ENDS WITH $tx_id_last_8_chars
                RETURN t.tx_id;
                """,
                in_total_amount=in_total_amount,
                out_total_amount=out_total_amount,
                tx_id_last_8_chars=tx_id_last_8_chars
            )
            single_result = result.single()
